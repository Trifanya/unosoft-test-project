...
mvn package
```
Для запуска готового jar-файла из корневой директории необходимо выполнить команду:
```
java -jar target/UnoSoftTestProject.jar "src/main/resources/input.txt"
```
Для запуска jar-файла из корневой директории с ограничением по памяти - 1 ГБ - необходимо выполнить компанду:
```
java -jar target/UnoSoftTestProject.jar "src/main/resources/input.txt" -Xmx1G
