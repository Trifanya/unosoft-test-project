# Тестовое задание для компании Уно-Софт.
## Задание
С помощью Java:
...
## Запуск
Для сборки jar-файла с помощью Maven необходимо в терминале выполнить команду: 
```
mvn package
```
Для указания файла, в который будет записан результат, необходимо указать полное имя 
файла в appication.properties (result-file-name)

Для запуска готового jar-файла необходимо выполнить команду:
```
java -jar путь-к-jar-файлу путь-к-тестовому-файлу
```
Для запуска jar-файла с ограничением по памяти - 1 ГБ - необходимо выполнить компанду:
```
java -Xmx1G -jar путь-к-jar-файлу путь-к-тестовому-файлу 
